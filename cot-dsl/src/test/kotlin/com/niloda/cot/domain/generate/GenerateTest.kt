...
class GenerateTest {
    ...
    fun conditional_true_renders_section() {
        ...
        val out = generate(cot, RenderParams.of(mapOf("enabled" to true)), { r,c, p -> renderConfigurable(r, c, p) })
    ...
    fun conditional_false_omits_section() {
        ...
        val out = generate(cot, RenderParams.of(mapOf("enabled" to false)), { r,c, p -> renderConfigurable(r, c, p) })
        val s = out.getOrElse { error("Generation failed: $it") }
        assertEquals("", s)
    }
    ...
    fun repetition_count_renders_n_times() {
        ...
        val out = generate(cot, RenderParams.of(mapOf("n" to 3)), { r,c, p -> renderConfigurable(r, c, p) })
        val s = out.getOrElse { error("Generation failed: $it") }
        assertEquals("AAA", s)
    }
    ...
    fun repetition_zero_renders_empty() {
        ...
        val out = generate(cot, RenderParams.of(mapOf("n" to 0)), { r,c, p -> renderConfigurable(r, c, p) })
        val s = out.getOrElse { error("Generation failed: $it") }
        assertEquals("", s)
    }
    ...
    fun oneOf_renders_selected_choice() {
        ...
        val out = generate(cot, RenderParams.of(mapOf("choice" to "b")), { r,c, p -> renderConfigurable(r, c, p) })
        val s = out.getOrElse { error("Generation failed: $it") }
        assertEquals("B", s)
    }
    ...
    fun missing_parameter_reports_error() {
        ...
        val res = generate(cot, RenderParams.empty(), { r,c, p -> renderConfigurable(r, c, p) })
        res.fold(
    ...
    fun type_mismatch_reports_error() {
        ...
        val res = generate(cot, RenderParams.of(mapOf("enabled" to "yes")), { r,c, p -> renderConfigurable(r, c, p) })
        res.fold(
    ...
    fun oneOf_unknown_key_reports_error() {
        ...
        val res = generate(cot, RenderParams.of(mapOf("choice" to "c")), { r,c, p -> renderConfigurable(r, c, p) })
        res.fold(
